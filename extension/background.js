/**
 * Chatgpt CLI Bridge - Background Service Worker
 *
...
function connectWebSocket() {
  ...
  ws.onopen = () => {
    console.log('[Chatgpt CLI Bridge] Connected to CLI server');
    isConnected = true;
    broadcastStatus();
  };

  ws.onclose = () => {
    console.log('[Chatgpt CLI Bridge] Disconnected from CLI server');
    isConnected = false;
    ws = null;
    broadcastStatus();
  };
  ...
  ws.onmessage = (event) => {
    try {
      ...
      console.error('[Chatgpt CLI Bridge] Failed to parse message:', e);
    }
  };
}
...
async function handleCommand(msg) {
  ...
  try {
    ...
    console.error(`[Chatgpt CLI Bridge] Error handling "${type}":`, error);
...
async function ensureContentScript(tabId) {
  ...
  try {
    ...
    console.debug('[Chatgpt CLI Bridge] Content script injection:', e.message);
  }
}
